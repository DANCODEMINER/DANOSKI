...
""")
        ...
        print(f"[ERROR] Error sending reset email: {e}")
        return False
        ...
        if self.path == '/submit_testimony':
            ...
            try:
                ...
                if not all([username, code, new_password]):
                    raise ValueError("Username, code, and new password are required")

                stored = reset_codes.get(username)
                ...
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"status": "success", "message": "Password reset successful."}).encode('utf-8'))
